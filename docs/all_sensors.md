...
- **Part I: Base Gateway & Network Entities**: The default virtual devices (comprising 130 entities across the Alerts, Gateway, Internet, Security, Speedtest, Status, and System sub-devices) present in standard gateway-monitoring mode.
...
>
> The default sub-device names are **UniFi Network Alerts / Gateway / Internet / Security / Speedtest / Status / System**; counts are for the base scenario: **130 entities** = Alerts 4 + Gateway 18 + Internet 41 + Security 20 + Speedtest 14 + Status 23 + System 10.
>
> **Enabled vs disabled — depends on HA Core UniFi:** the 130 registered entities are the same either way; only the enabled-by-default count differs.
>
...
| Name | Key | Type | Unit | Category | Notes | About |
| :-- | :-- | :-- | :-- | :-- | :-- | :-: |
...
### 3. Internet Sub-Device (41 Entities)
...
| WAN1 Projected Usage | `gateway_wan1_month_projected` | Sensor | GB | — | Projected end-of-month usage from usage so far. **No LTS (no `state_class`)** — it is a forecast, not a measurement. Judge it by its `confidence` attribute, which is weak early in the month. | ✓ |
...
| WAN2 Projected Usage | `gateway_wan2_month_projected` | Sensor | GB | — | Projected end-of-month usage from usage so far. **No LTS (no `state_class`)** — it is a forecast, not a measurement. Judge it by its `confidence` attribute, which is weak early in the month. Created only when dual-WAN monitoring is enabled. | ✓ |
...
### 4. Security Sub-Device (20 Entities)
...
| Rogue AP Proximity Alert | `gateway_rogue_proximity_alert` | Binary Sensor | — | Diagnostic |  | On when a rogue AP at or above the Proximity Threshold survives your band/SSID/AP ignore settings. |
...
| Rogue Detection Period | `rogue_period` | Select | — | Config |  | How far back each poll looks when counting current rogues. Separate from the on-demand Get Rogue APs action. |
...
| WAN1 Run | `gateway_wan1_speedtest` | Button | — | — | Data may not be available in all configurations. | Press to run an immediate speedtest on this WAN. Takes ~1 minute; results refresh shortly after. |
...
| WAN2 Run | `gateway_wan2_speedtest` | Button | — | — | Data may not be available in all configurations. | Press to run an immediate speedtest on this WAN. Takes ~1 minute; results refresh shortly after. |
...
### 7. System Sub-Device (10 Entities)
...
| Integration Health | `integration_health` | Binary Sensor | — | Diagnostic | Problem sensor; on when the integration self-diagnoses a degraded/malformed state. | On when the integration self-diagnoses a problem: a data source is unavailable (and not one you disabled), or a controller update appears to have changed the data format. See the attributes for details. |
...
>
> **`about` attribute:** **46 entities** carry an unrecorded `about:` attribute — a one-line explanation shown in More Info / Tools but excluded from the recorder (`_unrecorded_attributes`), so it costs nothing however often the state changes. The **About** column above marks them. The Strongest Rogue SSID `rogue_aps` list attribute is capped at 25 (`rogue_aps_truncated` flags overflow) and is also unrecorded.
>
> **Not every entity has one, deliberately.** A note on all 130 would train users to ignore notes, so the test is whether the entity's meaning is genuinely non-obvious from its name. The omissions below are decisions, not gaps:
>
...
> Every figure below moved by **+2** at `[1.0.1-dev16]` when the two **Projected Usage** sensors were added. They are enabled by default and do not duplicate anything Core UniFi provides, so they land in the registered _and_ both enabled columns in every mode.

| Mode (rig: 8 APs, 12 switches) | Devices | Registered | Enabled — no Core | Enabled — Core present |
| :-- | :-: | :-: | :-: | :-: |
| Don't add | 7 | 130 | 107 | 101 |
| AP Satisfaction Score only | 15 | 154 | n/a¹ | 109 |
| Add all device sensors | 27 | 290 | 267 | 109 |
...
| Name | Key | Type | Unit | Default (Core present) | Notes | About |
| :-- | :-- | :-- | :-- | :-- | :-- | :-: |
