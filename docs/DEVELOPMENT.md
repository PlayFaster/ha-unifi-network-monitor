...
The integration follows the standard Home Assistant Custom Component pattern, optimised for asynchronous performance.
...
- **`sensor.py`**: Declarative `value_fn` sensor engine for gateway, network health, per-AP, and per-switch sensors. Guard bands enforced at `native_value` time. Per-device creation gated by `unifi_device_mode` (`_device_descs`); gateway groups gated by `disabled_endpoints`; an `available` override + `_ENDPOINT_BY_KEY` map ties each sensor to its source endpoint; duplicate-of-core keys get a `_mon` entity_id.
- **`binary_sensor.py`**: Gateway connectivity/update/speedtest-pass sensors, network health OK sensors, dynamic WiFi/VPN status sensors, and the **Rogue AP Proximity Alert** (`PROBLEM`). Same `available`/endpoint-tag pattern as `sensor.py` (`_BINARY_ENDPOINT_BY_KEY`).
...
- **`switch.py`**: Pause Polling switch (`stop_polling` in options).
- **`config_flow.py`**: Initial setup, reconfigure, reauth, and options flows. Credentials never pre-filled in edit flows. Feature toggles grouped in a collapsible `sensor_groups` **section**; `_flatten_sections()` un-nests them. Reconfigure updates + aborts (the update listener owns the reload).
...
- **`services.yaml`**: `cleanup_unused_entities` (`dry_run`).
- **`const.py`**: All domain constants, default values, gateway model strings, option keys, and `EP_*` endpoint labels.
- **`diagnostics.py`**: Redacts sensitive fields (API key, password, username, host, gateway MAC, WAN local/public IPs, rogue-AP BSSIDs, ISP name/org) from diagnostics output via `async_redact_data` (recurses by key name).
...
Entities are grouped into 4 logical sub-devices (for the main system) and dynamic per-device sub-devices:
...
| **UniFi Network Internet** | `/stat/device` / `/stat/health` / Speedtest V2 API | Speedtest results (download, upload, ping, last run), active WAN routing interface, ISP monthly/daily data usage, network uptime/drops/latency, and external IP. |
| **UniFi Network System** | `/stat/device` / `/stat/health` / Config options | Polling configurations, multi-WAN load balancer mode/weights, multi-WAN routing modes, threat management toggles (Ad-blocking/Honeypot/IPS mode), and manual Refresh button. |
| **UniFi Network Status** | `/stat/health` / Integration endpoints | Live client counts (WLAN/LAN user/guest/iot counts), adopted device count, active/total config counts (rules, VPNs, VLANs, WiFi networks), individual WiFi & VPN tunnel binary statuses, and subsystem health indicators (All OK, WAN OK, Internet OK, WiFi OK, LAN OK). |
...
**Known simplification**: in standalone mode, the override enables _all_ per-device entities — including secondary/diagnostic ones (e.g. Model, User Ports, Guest Ports) that were disabled for reasons unrelated to native integration duplication. A future refinement could add a second description flag (e.g. `disabled_without_native: bool = False`) to distinguish "disabled to avoid duplication" from "disabled because it's secondary data regardless". For now, enabling everything in standalone is the accepted behaviour.
...
Two Core-present behaviours affect the **base** (gateway) entities, both verified live (2026-07-08):
...
Two axes of user control, stored in `entry.options`, all defaulting to preserve prior behaviour:
...
- **Feature toggles** (`enable_speedtest`, `enable_wan_usage`, `enable_security_monitoring`, default `True`) — each maps a group of gateway sensors **and** its endpoint(s). Disabling a group both stops creating its sensors and skips the API calls (`disabled_endpoints(options)` + `_optional()` substitute a no-op fetch, preserving the positional `gather` unpack).
...
- **Gotcha — Rogue AP "Age" is Frozen and Not Relative to Present Time**:
  - **API Behavior**: The `/proxy/network/api/s/{site}/stat/rogueap` endpoint returns a static `"age"` integer field. This value represents the age of the detection *at the moment the scan report was generated by the AP* (computed as `report_time - last_seen` in seconds).
...
- **[2026-06-22]** — Added Pause Polling toggle switch (persisted to `entry.options`, survives restarts; resumes trigger immediate coordinator refresh). Added host URL normalisation in all config flow steps (`_clean_host()` strips `http://`/`https://` prefix and trailing slashes before storing). Per-device entity standalone detection: when native `unifi` integration is absent, all AP/switch entities enable by default instead of creating empty disabled sub-devices.
