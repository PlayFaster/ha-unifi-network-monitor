...
- The **current (open) bucket is re-apportioned on every poll.** A recompute can land slightly _below_ the previous reading — observed drops of 0.001 %–0.10 % (a few MB on a multi-GB total). This is upstream behavior, not a bug in our parsing: `_safe_float`'s 3-dp rounding reproduces the logged values exactly, and daily and monthly (independent endpoints) dropped by byte-identical amounts, proving a shared upstream restatement.
...
**Verified against real output, 2026-08-08.** This section previously described intent that had never been checked against a populated capture — and the module carried `diagnostics: done` through two IQS scans while leaking real identifiers, so source-reading is not acceptable evidence here. A capture with 95 gateway keys, 20 devices, 5 rogue-history entries and a live alert block was scanned end to end: **0 raw MACs and 0 private IPs**, keys and device names tokenized, `bssid` redacted, the `DEVICE` block reduced to its `gateway` label, `CONSOLE_NAME` blanked, and the alert **free text** scrubbed inline. Re-verify the same way after any change here.
...
**Why "delete and re-add" does not fix it** (verified against HA 2026.7.2 source — full research in `shared/SharedNotes/info/post_deletion_data/post_deletion_data_status.md`): the obvious workaround for a changed environment is to delete the integration and add it again so the `standalone` check re-runs. **It doesn't work.** Deleting a config entry does not discard the entity registry entries — HA moves them to `deleted_entities` inside `.storage/core.entity_registry` and keeps them for **30 days** (`ORPHANED_ENTITY_KEEP_SECONDS = 2592000`, swept every `CLEANUP_INTERVAL = 86400`). On re-add, `async_get_or_create()` matches on `(domain, platform, unique_id)` and restores `disabled_by` along with `entity_id`, `name`, `icon`, `area_id`, `labels` and `options`. A restored entity is **not** a first registration, so `entity_registry_enabled_default` is never consulted — the previous enabled/disabled state simply comes back. Our `unique_id`s are gateway-MAC-derived and therefore stable across a delete/re-add of the same hardware, so this triggers every time. (One nuance: only the `CONFIG_ENTRY` disabler is unwound on restore — a _user_-disabled entity stays disabled.)

Consequences worth knowing before recommending a reinstall to anyone:

- The real remedies are: toggle the affected entities manually (what the limitation already says), wait out the 30 days, or scrub the registry with HA stopped.
- **Delete/re-add is actively risky as a troubleshooting step.** `entity_id` is only restored if the slot is still free (`_entity_id_available`); if it isn't — or if more than 30 days elapsed — entities register as `…_2`, the **long-term statistics of the old `entity_id` are orphaned permanently** (LTS is never purged; it becomes a standing `no_state` issue in Tools → Statistics, removable only by hand), and dashboards/automations referencing the old ids break.
- Conversely, within the window a re-add is largely seamless: statistics and state history reconnect because they are keyed by `entity_id`. Monitor's own `.storage` files are the exception — `async_remove_entry` deletes them, so rogue-AP history genuinely restarts.
...
### Gateway Coexistence: Separate Devices + `enable_in_standalone`

> **Changed (2026-07):** Monitor no longer merges its device cards with Core UniFi. Device merging depended on a shared MAC `connections`, which HA 2026.8 stopped honouring; carrying it would have made ≤2026.7 merge and 2026.8+ not — two behaviors. Monitor now registers **identity-only** devices (`identifiers={(DOMAIN, …)}`, no `connections`) that never merge, uniformly on every HA version. See `.notes/device_registry/device_model_2026_08.md` and the _Device Co-existence_ section in `AGENTS.md`.

Two things govern the **base** (gateway) entities:

- **Separate gateway device — no merge.** `build_gateway_device_info` sets `identifiers={(DOMAIN, gateway_mac)}` only. Monitor's Gateway device is its own card showing **Monitor's** discovered name/model and the Network-application firmware version (`sw_version` from `entry.data`) — it does not adopt Core's card, name, or UniFi-OS version. The other sub-devices hang off it via `via_device_id` (2026.8+) / the `via_device` tuple (≤2026.7), through `_compat.via_device_link`. AP/switch devices are likewise separate. Core UniFi, if installed, keeps its own separate cards — co-existence is **separate but aware**, not merged.
- **`enable_in_standalone` gateway sensors.** Six gateway diagnostics carry `enable_in_standalone=True` (`cpu`, `ram`, `cpu_temp`, `board_temp`, `uptime` in `sensor.py`; `update_available` in `binary_sensor.py`). The gateway entity classes force these enabled **only** when standalone; with Core present they revert to `entity_registry_enabled_default=False` (disabled). Intent is unchanged by the no-merge switch: when Core is present it already surfaces the gateway's CPU, memory, and temperature sensors, so Monitor disables its duplicates by default to avoid redundant entities/polling (the user can enable them on the Monitor Gateway device if wanted). Net base enabled-by-default: **93 without Core / 87 with Core** (116 registered either way — enabled-default logic is entity-level and unaffected by the device-model change).
...
Full design in `.notes/design_monitor_setup_options.md`; cross-project porting guide in `shared/SharedNotes/issues/x_project/setup_cleanup_options.md`.
...
- **[2026-08-08]** — **August update cycle, phases 0–7** (`[1.0.1-dev10]` … `[1.0.1-dev22]`; per-phase detail in `docs/changelog_local.md`). Feature work: **Projected Usage** (`calendar_cycle_bounds` + `project_cycle_usage` in `helpers.py`, credibility-weighted blend of current run-rate against the prior cycle, with confidence bands travelling as attributes rather than as an unknown state); the **WAN weight write path hardened** — the pair is written under `asyncio.shield` and confirmed, returning a `WriteOutcome` of `confirmed`/`unverified`/`failed`, after tracing showed a cancelled debounce could suspend the task _between_ the two PUTs and leave the weights not summing to 100 with nothing reporting it; **debounced number writes** that flush rather than cancel on entity removal; and **repair issues scoped per entry**.
- **[2026-08-08]** — **Test baseline: coverage is necessary and not sufficient.** The suite went 624 → 842 at **100% line and 100% branch, 0 partials** (five genuinely unreachable defensive branches were deleted rather than left to erode the number), and zero-assertion tests 7 → 0. Mutation testing was then configured (`.validate/mutmut_modules.txt`, module list decided **by measurement** — a module whose tests mock its collaborator produces survivors that are noise by construction) and found what coverage could not: five boundary/arithmetic gaps in the new projection code, all the same mistake of **asserting an answer was plausible rather than right**, and 87 survivors in `diagnostics.py` traced to one cause — the tests asserted the output carried _no secret_ and never that it carried the _right token_. A scrub that is skipped or nulled satisfies an absence-only assertion. Final run: 840 killed / 205 survived of 1045, with every remaining `diagnostics.py` survivor carrying a written verdict in `.notes/issues/testing_deeper/mutation_equivalents.md`.
- **[2026-08-08]** — **Refresh Now reports failure.** `RefreshButton.async_press` awaited the **debounced** `async_request_refresh`, which records failure on `last_update_success` but does not propagate it — so the button could not raise and an automation pressing it always reported success, even with the gateway unreachable. Added `async_force_refresh_now` (non-debounced, so the outcome is known when it returns); the button reads `last_update_success` and raises `HomeAssistantError`. **`_force_refresh_once` remains consumed on attempt, not on success**: consuming on success re-arms the pause bypass until a fetch works, which turns a paused integration into a polling one for as long as the gateway stays down. Consequence for users, documented in the README: an action that raises stops the rest of an automation, so the examples that press a button before notifying now carry `continue_on_error: true` — without it a WAN-outage automation would abort on the refresh and never send its alert.
