...
> **Changed (2026-07):** Monitor no longer merges its device cards with Core UniFi. Device merging depended on a shared MAC `connections`, which HA 2026.8 stopped honouring; carrying it would have made ≤2026.7 merge and 2026.8+ not — two behaviours. Monitor now registers **identity-only** devices (`identifiers={(DOMAIN, …)}`, no `connections`) that never merge, uniformly on every HA version. See `.notes/device_registry/device_model_2026_08.md` and the _Device Co-existence_ section in `AGENTS.md`.
